...
Este proyecto implementa un cliente FTP, en consecuencia, necesitaremos intalar un servidor FTP concurrente.
**Instalación y Configuración:**
Para entornos Unix basados en Debian o Ubuntu la instalación se realiza con el siguiente comando:
```bash
$ sudo apt install vsftpd
```
Para hacer uso del servidor necesitamos iniciar su `Deamon` de la siguiente manera:
```bash
...
```
2. Ejecutar el archivo de configuración **Makefile** (Por cada cambio que querramos realizar en el codigo `TCPftp.c`)
```bash
$ make 
```
...
El cliente requiere ser iniciado con la direccion IP y puerto del servicio (valores predeterminados => localhost:21)
...
Este cliente hace el intento de implementar concurrencia por parte del cliente, ejecutando la consola de comandos desde un el socket de control y realizadno cargas o descargas concurrentes mediante el uso subprocesos en el socket de datos.
Sin embargo, se da el problema de que no se pueden realizar varias acciones de descarga y carga al mismo tiempo puesto que un solo socket de datos no puede realizar la operación concurrente.  

Este problema sera detallado en el informe.


Aquí tienes tu README corregido **solo en ortografía y pequeños errores de markdown**, sin modificar el formato ni el contenido técnico:
